...
const DEFAULT_API_KEY_CALL_LIMIT = 3;

class FlightAPI {
  constructor(apiKey, timeout = NETWORK_TIMEOUT) {
    this.apiKey = apiKey;
    this.timeout = timeout;
  }

  async fetchFlightsSchedule(airportCode) {
    const apiKeyFromStorage = sessionStorage.getItem('apiKey') || localStorage.getItem('apiKey');
    let apiKeyToUse = this.apiKey;

    if (apiKeyFromStorage) {
      apiKeyToUse = apiKeyFromStorage;
    } else {
      const apiCallCount = parseInt(localStorage.getItem('apiCallCount') || '0', 10);
      if (apiCallCount >= DEFAULT_API_KEY_CALL_LIMIT) {
        return { status: 'error', error: 'API call limit reached. Please use your own API key.' };
      }
      localStorage.setItem('apiCallCount', apiCallCount + 1);
    }
    const url = `https://api.flightapi.io/compschedule/${apiKeyToUse}?mode=arrivals&iata=${airportCode}&day=1`;
...
class FlightDataProcessor {
  static extractArrivals(dataRecord) {
    try {
      ...
      const timezoneOffsetHrs =
        pluginData?.details?.timezone?.offset / 3600 || 0;
  ...
  static extractFlightId(arrival, timezoneOffsetHrs) {
    try {
      ...
      const countryName =
        arrival?.flight?.airport?.origin?.position?.country?.name;
      ...
      if (!timeArrival || !flightNumber || !countryName) {
        return null;
      }
      ...
      console.error('Error extracting flight ID:', error);
      return null;
    }
  }
  ...
  static getFlightsCountByCountry(data) {
    ...
    data.forEach((dataRecord) => {
      const [arrivals, timezoneOffsetHrs] =
        FlightDataProcessor.extractArrivals(dataRecord);

      arrivals.forEach((arrival) => {
        const flightId = FlightDataProcessor.extractFlightId(
          arrival,
          timezoneOffsetHrs
        );
        if (flightId) {
...
document
  ...
  .addEventListener('submit', async function (event) {
    ...
    const flightsCountByCountry = FlightDataProcessor.getFlightsCountByCountry(
